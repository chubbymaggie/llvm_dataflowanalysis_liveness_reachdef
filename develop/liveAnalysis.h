...
namespace {
	...
	class LiveAnalysis: public IDFA<FlowType> {
		...
		LiveAnalysis() : IDFA<FlowType>() {
			//BBtoInfo = (IDFA<FlowType>::getBBtoInfo());
			//InstToInfo = (IDFA<FlowType>::getInstToInfo());
			//domainToIdx = (IDFA<FlowType>::getdomainToIdx());
		}
		//meet operator
		...
		virtual void initInstGenKill(Instruction *ii);
		//generate the gen and kill set for each block
		virtual void initGenKill(BasicBlock *Bi, BasicBlock *Pi);
		//get the boundary condition
		virtual BitVector* getBoundaryCondition(int len, Function &F);
	 ...
	 */
	template <class FlowType>
	void LiveAnalysis<FlowType>::initInstGenKill(Instruction *ii) {
		idfaInfo *instInf = IDFA<FlowType>::InstToInfo[ii];
		User::op_iterator OI, OE;
		for (OI = ii->op_begin(), OE = ii->op_end(); OI != OE; ++OI) {
			Value *val = *OI;
			if (isa<Instruction>(val) || isa<Argument>(val)) {
				int valIdx = IDFA<FlowType>::domainToIdx[val];
				(instInf->gen)->set(valIdx);
			}
		}
		ValueMap<Value*, unsigned>::const_iterator iter = IDFA<FlowType>::domainToIdx.find(dyn_cast<Instruction>(ii));
		if (iter != IDFA<FlowType>::domainToIdx.end()) {
			Value *val = ii;
			int valIdx = IDFA<FlowType>::domainToIdx[val];
	 ...
	 */
	template <class FlowType>
	void LiveAnalysis<FlowType>::initGenKill(BasicBlock *Bi, BasicBlock *Pi) {
		idfaInfo *BBinf = IDFA<FlowType>::BBtoInfo[&*Bi];
		(BBinf->gen)->reset(0, IDFA<FlowType>::domainToIdx.size());
		(BBinf->kill)->reset(0, IDFA<FlowType>::domainToIdx.size());	
		//Gen set
		for (BasicBlock::iterator ii = Bi->begin(), ie = Bi->end(); ii != ie; ++ii) {
			if(isa<PHINode>(ii)){
				...
				if (idx >= 0 && idx < pN->getNumIncomingValues()) {
					...
					if (isa<Instruction>(val) || isa<Argument>(val)) {
						unsigned valIdx = IDFA<FlowType>::domainToIdx[val];
	 ...
	 */
	template <class FlowType>
	BitVector* LiveAnalysis<FlowType>::getBoundaryCondition(int len, Function &F) {
